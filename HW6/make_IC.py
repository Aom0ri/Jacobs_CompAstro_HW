...
def make_IC():
    ...
    N_1D=200; # 1D particle number (so total particle number is N_1D^DIMS)
    fname='sod1d_ics.hdf5'; # output filename 
    x0=np.linspace(0,2,200);
    Lbox = 1.0 # box side length
    rho_desired = np.array([1 if i <= 0.75 else 0.125 for i in x0[:-1]]) # box average initial gas density
    P_desired = np.array([1 if i <= 0.75 else 0.1 for i in x0[:-1]]) # initial gas pressure
    vgrainrms = 0.0 # rms velocity of collisionless particles
    dust_to_gas_ratio = 0.01 # mass ratio of collisionless particles to gas
    gamma_eos = 1.4 # polytropic index of ideal equation of state the run will assume
